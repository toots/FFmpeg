...
static void prompegdec_set_next_index(
    PrompegDecoder *decoder, uint16_t next_index);
...
PrompegDecoder *ff_prompegdec_alloc_decoder(
    URLContext *url_context, PrompegDecoderConfig *config)
{
    ...
    memcpy(&decoder->config, config, sizeof(PrompegDecoderConfig));

    decoder->restore_buffer =
        av_mallocz(sizeof(PrompegDecoderPacket *) * MIN(config->l, config->d));
    if (!decoder->restore_buffer)
        goto fail;

    decoder->tmp_bitstring = av_malloc(config->bitstring_size);
    if (!decoder->tmp_bitstring)
        goto fail;

    decoder->bitstring = av_malloc(config->bitstring_size);
    ...
    PrompegDecoderPacketType type, PrompegDecoderPacket *packet)
{
    ...
    default:
        ...
        if (!decoder->started && decoder->next_index <= packet->index) {
            decoder->started =
                decoder->config.min_packets <= decoder->packets_count;
            prompegdec_set_next_index(decoder, packet->index);
        }
    }
    ...
    int (*cmp)(void *opaque, void *elem), int (*enu)(void *opaque, void *elem))
{
    ...
    fec_enum.l = decoder->config.l;
    fec_enum.d = decoder->config.d;
    ...
    PrompegDecoderPacket *fec_packet)
{
    ...
    ff_prompeg_pack_fec_bitstring(
        decoder->bitstring, fec_packet->bytes, decoder->config.fec_packet_size);
    ...
    for (i = 0; i < buffer_length; i++) {
        if (decoder->restore_buffer[i]) {
            ...
            ff_prompeg_pack_bitstring(decoder->tmp_bitstring,
                decoder->restore_buffer[i]->bytes, decoder->config.packet_size);
            ff_prompeg_xor_fast(decoder->bitstring, decoder->tmp_bitstring,
                decoder->bitstring, decoder->config.bitstring_size);
        }
    }

    packet = prompegdec_alloc_packet(index, decoder->config.packet_size);
    if (!packet)
        return AVERROR(ENOMEM);

    ff_prompeg_restore_packet(packet->bytes, decoder->bitstring, m, ssrc, index,
        decoder->config.bitstring_size);
    ...
    PrompegDecoder *decoder, PrompegDecoderPacket *fec_row_packet)
{
    ...
    if (packets_count != decoder->config.l - 1)
        return 0;

    ret = prompeg_restore_packets_buffer(decoder, missing_index,
        decoder->config.l, PROMPEGDEC_FEC_ROW_PACKET, fec_row_packet);
    ...
    PrompegDecoder *decoder, PrompegDecoderPacket *fec_col_packet)
{
    ...
    for (i = 0; i < decoder->config.d; i++) {
        index = fec_col_packet->index + i * decoder->config.l;
        decoder->restore_buffer[i] =
            prompegdec_find_packet(decoder->packets, index);
    ...
    if (packets_count != decoder->config.d - 1)
        return 0;

    ret = prompeg_restore_packets_buffer(decoder, missing_index,
        decoder->config.d, PROMPEGDEC_FEC_COL_PACKET, fec_col_packet);
...
static int prompegdec_restore_fec_matrix(PrompegDecoder *decoder)
{
    ...
    do {
        ...
        for (i = 0; i < decoder->config.d; i++) {
            packet_index =
                decoder->first_fec_packet_index + i * decoder->config.l;
...
{
    int16_t first_fec_packet_index;
    ...
    if (decoder->next_fec_row && decoder->next_fec_col)
        first_fec_packet_index = decoder->next_fec_col->index -
                                 decoder->next_index +
                                 decoder->next_fec_row->index;
    else
        first_fec_packet_index =
            decoder->next_index - decoder->config.l * decoder->config.d;

    decoder->first_fec_packet_index = (uint16_t)first_fec_packet_index;
}
...
static void prompegdec_set_next_index(
    PrompegDecoder *decoder, uint16_t next_index)
{
    decoder->next_index = next_index;
    decoder->next_fec_row = decoder->next_fec_col = NULL;

    prompegdec_populate_fec_data(decoder);

    decoder->min_discard_new_packet_index =
        decoder->next_index + decoder->config.max_packet_gap;
    decoder->max_discard_old_packet_index = decoder->first_fec_packet_index;

    if (decoder->first_fec_packet_index < decoder->min_discard_new_packet_index)
    {
        decoder->max_discard_new_packet_index = INT32_MAX;
        decoder->min_discard_old_packet_index = 0;
        decoder->max_discard_old_packet_index = decoder->first_fec_packet_index;
    } else {
        decoder->max_discard_new_packet_index = decoder->first_fec_packet_index;
        decoder->min_discard_old_packet_index =
            decoder->min_discard_new_packet_index;
        decoder->max_discard_old_packet_index =
            decoder->max_discard_new_packet_index;
    }

    av_log(decoder, AV_LOG_DEBUG,
        "Next packet index: %d, first FEC packet: %d, discard old packets: "
        "[%d,%d), discard new packets: [%d,%ld)\n",
        decoder->next_index, decoder->first_fec_packet_index,
        decoder->min_discard_old_packet_index,
        decoder->max_discard_old_packet_index,
        decoder->min_discard_new_packet_index,
        (long)decoder->max_discard_new_packet_index);
}
    ...
    PrompegDecoder *decoder, PrompegDecoderPacket *packet, uint8_t *bytes)
{
    ...
    memcpy(bytes, packet->bytes, decoder->config.packet_size);
    prompegdec_set_next_index(decoder, packet->index + 1);
    decoder->pending_packets--;

    do {
        old_packet = prompegdec_get_first_packet(decoder->packets,
            decoder->min_discard_old_packet_index,
            decoder->max_discard_old_packet_index);
    ...
    do {
        old_packet = prompegdec_get_first_packet(decoder->fec_col_packets,
            decoder->min_discard_old_packet_index,
            decoder->max_discard_old_packet_index);
    ...
    do {
        old_packet = prompegdec_get_first_packet(decoder->fec_row_packets,
            decoder->min_discard_old_packet_index,
            decoder->max_discard_old_packet_index);
    ...
    return decoder->config.packet_size;
}

static int prompegdec_accept_packet(PrompegDecoder *decoder, uint16_t index)
{
    if (!decoder->started)
        return 1;

    if (decoder->min_discard_old_packet_index <= index &&
        index < decoder->max_discard_old_packet_index)
        return 0;

    if (decoder->min_discard_new_packet_index <= index &&
        index < decoder->max_discard_new_packet_index)
        return 0;
...
{
    PrompegDecoderPacket *packet;
    int expected_length = type == PROMPEGDEC_PACKET
                              ? decoder->config.packet_size
                              : decoder->config.fec_packet_size;

    if (length != expected_length)
        return AVERROR(EINVAL);

    if (!prompegdec_accept_packet(decoder, index))
        return 0;

    switch (type) {
    case PROMPEGDEC_FEC_ROW_PACKET:
        if (decoder->config.max_fec_packets <= decoder->fec_row_packets_count) {
    ...
    case PROMPEGDEC_FEC_COL_PACKET:
        if (decoder->config.max_fec_packets <= decoder->fec_col_packets_count) {
            av_log(decoder, AV_LOG_ERROR,
    ...
    PrompegDecoder *decoder, uint8_t *bytes, int length)
{
    ...
    if (length < decoder->config.packet_size)
        return AVERROR(EINVAL);

    if (!decoder->started || !decoder->pending_packets)
    ...
    if (decoder->packets_count < decoder->config.max_packets)
        return AVERROR(EAGAIN);
